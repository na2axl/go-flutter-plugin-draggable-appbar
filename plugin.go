...
func (p *Plugin) InitPlugin(messenger plugin.BinaryMessenger) error {
	...
	channel.HandleFunc("focus", p.focus)
	channel.HandleFunc("getCursorPosition", p.getCursorPosition)
	channel.HandleFunc("getHeight", p.getHeight)
	channel.HandleFunc("getPosition", p.getPosition)
	channel.HandleFunc("getPositionX", p.getPositionX)
	channel.HandleFunc("getPositionY", p.getPositionY)
	channel.HandleFunc("getSize", p.getSize)
	channel.HandleFunc("getTitle", p.getTitle)
	channel.HandleFunc("getWidth", p.getWidth)
	channel.HandleFunc("hide", p.hide)
	channel.HandleFunc("iconify", p.iconify)
	channel.HandleFunc("maximize", p.maximize)
	channel.HandleFunc("maximize", p.maximize)
	channel.HandleFunc("onDragEnd", p.onDragEnd)
	channel.HandleFunc("onDragStart", p.onDragStart)
	channel.HandleFunc("restore", p.restore)
	channel.HandleFunc("setCursorPosition", p.setCursorPosition)
	channel.HandleFunc("setPosition", p.setPosition)
	channel.HandleFunc("setSize", p.setSize)
	channel.HandleFunc("setTitle", p.setTitle)
	channel.HandleFunc("show", p.show)
	return nil
}
